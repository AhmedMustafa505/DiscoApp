...
function WorkBudgetScreen(props) {
  ...
  const workTransactions = useMemo(() => {
    return transactions.filter((transaction) =>
      ["Transportation", "Business"].includes(transaction.category)
    );
  }, [transactions]);
  ...
  const filteredTransactions = workTransactions.filter((transaction) => {
...
export default WorkBudgetScreen;
